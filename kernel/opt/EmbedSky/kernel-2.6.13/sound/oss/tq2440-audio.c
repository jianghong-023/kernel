...
#define AUDIO_NAME		"TQ2440_UDA1341"
#define AUDIO_NAME_VERBOSE	"TQ2440 UDA1341 audio driver"
...
static void     start_tq2440_iis_bus_tx(void);
static void     start_tq2440_iis_bus_rx(void);
...
static          ssize_t
tq2440_audio_write(struct file *file, const char *buffer, size_t count, loff_t * ppos)
{
...
static          ssize_t
tq2440_audio_read(struct file *file, char *buffer, size_t count, loff_t * ppos)
{
    const char     *buffer0 = buffer;
...
static unsigned int
tq2440_audio_poll(struct file *file, struct poll_table_struct *wait)
{
    unsigned int    mask = 0;
    int             i;
...
static          loff_t
tq2440_audio_llseek(struct file *file, loff_t offset, int origin)
{
    return -ESPIPE;
}

static int
tq2440_mixer_ioctl(struct inode *inode, struct file *file, uint cmd, ulong arg)
{
    /*
...
static int
tq2440_audio_ioctl(struct inode *inode, struct file *file, uint cmd, ulong arg)
{
    long            val;
...
//printk("a_ioctl = 0x%08x, SNDCTL_DSP_GETOSPACE=0x%08x\n", cmd, SNDCTL_DSP_GETOSPACE);
    ...
    case SNDCTL_DSP_SETDUPLEX:
        ...
        return -ENOSYS;
    default:
        return tq2440_mixer_ioctl(inode, file, cmd, arg);
    }
    return 0;
}

static int
tq2440_audio_open(struct inode *inode, struct file *file)
{
    ...
    if (cold)
    {
        ...
        if ((file->f_mode & FMODE_WRITE))
        {
            ...
            start_tq2440_iis_bus_tx();
            audio_clear_buf(&output_stream);
//	    if (!output_stream .buffers && audio_setup_buf(&output_stream))		//HJ
//		return -ENOMEM;		//HJ
...
static int
tq2440_audio_release(struct inode *inode, struct file *file)
{
...
static struct file_operations tq2440_audio_fops = {
  llseek:  tq2440_audio_llseek,
  write:   tq2440_audio_write,
  read:    tq2440_audio_read,
  poll:    tq2440_audio_poll,
  ioctl:   tq2440_audio_ioctl,
  open:    tq2440_audio_open,
  release: tq2440_audio_release,
  owner:	THIS_MODULE
};

static struct file_operations tq2440_mixer_fops = {
  ioctl: tq2440_mixer_ioctl,
  owner: THIS_MODULE
};
...
#ifdef CONFIG_PM
static int
tq2440_audio_suspend(struct device *dev, u32 state, u32 level)
{
    switch (level)
    {
...
static int
tq2440_audio_resume(struct device *dev, u32 level)
{
    switch (level)
    {
    case RESUME_POWER_ON:
        printk("%s\n", __FUNCTION__);
        tq2440_uda1341_enable();
        break;
    }
...
#else
#define tq2440_audio_suspend	NULL
#define tq2440_audio_resume	NULL
#endif
...
static int tq2440_audio_probe(struct device *_dev)
{
    int             ret = 0;

    printk("TQ2440 SOUND driver probe!\n");
...
static int tq2440_audio_remove(struct device *_dev)
{
...
static struct device_driver tq2440_audio_driver = {
	.name		= "s3c2440-sound",
	.bus		= &platform_bus_type,
	.probe		= tq2440_audio_probe,
	.remove		= tq2440_audio_remove,
	.suspend		= tq2440_audio_suspend,
	.resume		= tq2440_audio_resume,
};

#ifdef USE_SYSFS
static int __init tq2440_uda1341_init(void)
{
	int ret = -ENODEV;
printk("TQ2440 SOUND driver register\n");
	//if (!machine_is_h3600() || machine_is_h3100() || machine_is_h3800())
	ret = driver_register(&tq2440_audio_driver);
	if( ret )
	{
...
static void __exit tq2440_uda1341_exit(void)
{
	driver_unregister(&tq2440_audio_driver);
}
#else
...
void __exit tq2440_uda1341_exit(void)
{
    unregister_sound_dsp(audio_dev_dsp);
    unregister_sound_mixer(audio_dev_mixer);
    audio_clear_dma(&output_stream);
...
module_init(tq2440_uda1341_init);
module_exit(tq2440_uda1341_exit);
