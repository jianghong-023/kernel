...
static void uda1341_sync(struct l3_client *clnt)
{
	...
	if (p != buf)
	{
		ret = l3_write(clnt, UDA1341_DATA0, buf, p - buf);
		printk("ret = %d\n", ret);
	}
	#else
	if (p != buf)
...
//printk("ghcstop=====================, uda3141_cmd_init\n");
    ...
    #if 0
	ret = l3_write(clnt, UDA1341_STATUS, buf, 2);
	#else
	...
	//printk("data0_0 = 0x%02X\n", uda->regs.data0_0);
	
	//ret = l3_read(clnt, UDA1341_DATA0, buf, 1);
	//printk("ret = %d, 0x%02X, %02X\n", ret, buf[0], buf[1]);
...
static int uda1341_attach(struct l3_client *clnt)
{
...
#ifdef CONFIG_CPU_S3C2440
	#if (S_CLOCK_FREQ == 384)
	//printk("=========================ghcstop uda 1341 attatch\n");
	uda->regs.stat0   = STAT0_SC_384FS | STAT0_IF_MSB;
	#else
	uda->regs.stat0   = STAT0_SC_256FS | STAT0_IF_MSB;
	#endif
#else
	...
	uda->regs.data0_0 = DATA0_VOLUME(2);
	uda->regs.data0_1 = DATA1_BASS(0) | DATA1_TREBLE(0);
	uda->regs.data0_2 = DATA2_PEAKAFTER | DATA2_DEEMP_NONE | DATA2_FILTER_MAX;
	uda->regs.ext0    = EXT0_CH1_GAIN(6);
