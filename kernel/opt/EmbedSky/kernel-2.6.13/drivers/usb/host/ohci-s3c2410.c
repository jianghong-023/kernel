...
#include <asm/arch/regs-gpio.h>		//HJ
...
{
	printk("Initial TQ2440 TWO USB HOST Driver!\n");
#ifdef CONFIG_TQ2440_TWO_USB_HOST
	unsigned long reg_misccr;
	reg_misccr = 0;
	reg_misccr = __raw_readl(S3C2410_MISCCR);
#ifdef debug
	printk("MISCCR:0x%08lx\n",reg_misccr);
#endif
	reg_misccr = reg_misccr | S3C2410_MISCCR_USBHOST;
	__raw_writel(reg_misccr,S3C2410_MISCCR);
#ifdef debug
	printk("MISCCR:0x%08lx\n",reg_misccr);
#endif
#endif
