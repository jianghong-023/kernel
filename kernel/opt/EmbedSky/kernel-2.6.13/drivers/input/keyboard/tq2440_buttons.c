...
#define DEVICE_NAME     "TQ2440_buttons"		/* 加载模式后，执行”cat /proc/devices”命令看到的设备名称 */
...
/* 应用程序对设备文件/dev/TQ2440-buttons执行open(...)时，
 * 就会调用tq2440_buttons_open函数
 */
...
**************************************************/
static unsigned int tq2440_buttons_poll( struct file *file, struct poll_table_struct *wait)
{
	unsigned int mask = 0;
...
static struct file_operations tq2440_buttons_fops =
{
	...
	.poll	=	tq2440_buttons_poll,
};

/*
 * 执行“insmod tq2440_buttons.ko”命令时就会调用这个函数
 */
static int __init tq2440_buttons_init(void)
{
	 ...
	 * 操作主设备为BUTTON_MAJOR的设备文件时，就会调用tq2440_buttons_fops中的相关成员函数
	 * BUTTON_MAJOR可以设为0，表示由内核自动分配主设备号
	*/
	ret = register_chrdev(BUTTON_MAJOR, DEVICE_NAME, &tq2440_buttons_fops);
	if (ret < 0)
	{
...
/*
 * 执行”rmmod tq2440_buttons.ko”命令时就会调用这个函数 
 */
...
MODULE_DESCRIPTION("TQ2440 BUTTON Driver");		// 一些描述信息
