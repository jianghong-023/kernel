/*
 *  linux/drivers/mmc/s3c2440mci.h - TQ2440 SDI Interface driver
 *
 *  Copyright (C) 2008 www.embedsky.net, All Rights Reserved.
 *
...
//	while (readl(host->base + S3C2410_SDIDSTA) &
		if (readl(host->base + S3C2410_SDIDSTA) &
...
static int s3cmci_no_card(struct s3cmci_host *host)
{
...
static void s3cmci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
   ...
   if (s3cmci_no_card(host) == 0) {
//      dbg(host, dbg_err, "%s: no card\n", __func__);
...
static int s3cmci_probe(struct device *dev, int is2440)
{
...
static int s3cmci_remove(struct device *dev)
{
...
static int s3cmci_probe_2440(struct device *dev)
{
	return s3cmci_probe(dev, 1);
}

static struct device_driver s3cmci_driver_2440 = {
	.name		 "TQ2440-SD",
...
{
	return driver_register(&s3cmci_driver_2440);
}

static void __exit s3cmci_exit(void)
{
	driver_unregister(&s3cmci_driver_2440);
}

module_init(s3cmci_init);
module_exit(s3cmci_exit);

MODULE_DESCRIPTION("TQ2440 Multimedia Card Interface driver");
MODULE_LICENSE("GPL");
