...
#if defined CONFIG_MACH_SCB9328 || defined CONFIG_ARCH_FS_PXA255	//modified by hzh
...
#define DM9000_WORD_MODE
#endif

#ifdef DM9000_BYTE_MODE
static inline void
dmfe_imx_ins(u_long ioaddr, u_char * buf, int len)
{
	u16 i;

	for (i = 0; i < len; i++)
		buf[i] = DM9000_inb(ioaddr);
}
#elif defined DM9000_WORD_MODE
static inline void
dmfe_imx_ins(u_long ioaddr, u_char * buf, int len)
{
	u16 tmplen, i;
	...
	tmplen = (len + 1) / 2;
	for (i = 0; i < tmplen; i++)
		((u16 *) buf)[i] = DM9000_inw(ioaddr);
}
#elif defined DM9000_DWORD_MODE
static inline void
dmfe_imx_ins(u_long ioaddr, u_char * buf, int len)
{
	u16 tmplen, i;
