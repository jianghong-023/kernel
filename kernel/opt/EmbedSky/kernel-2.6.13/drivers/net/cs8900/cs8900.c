...
#if defined(CONFIG_ARCH_SMDK2410)|defined(CONFIG_ARCH_TQ2440)
...
int __init cs8900_probe (struct net_device *dev)
{
	...
	int i,result;
	u16 value;
    unsigned int dwBWSCONVal;

	printk (VERSION_STRING"\n");

	memset (&priv,0,sizeof (cs8900_t));

    dwBWSCONVal = __raw_readl(S3C2410_BWSCON);
    dwBWSCONVal = (dwBWSCONVal & ~0xf000) | 0xd000;
	__raw_writel(dwBWSCONVal,S3C2410_BWSCON);
